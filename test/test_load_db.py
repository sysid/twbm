...
def db(request, setup):
    ...
    def teardown():
        _log.info("teardown")
        db.conn.commit()  # delay_commit = True
        db.cur.close()
        db.conn.close()
...
def test_traverse_bm_folder(db, data):
    ...
    for item in db.traverse_bm_folder(bookmark_bar['children'], unique_tag=None, folder_name='Bookmarks bar', add_parent_folder_as_tag=True):
